-- AIRS ledger identities are UUIDs and the prior text RPC immediately cast the
-- identifier to UUID. Keep one UUID signature so PostgREST has an unambiguous
-- RPC contract and cannot retain a stale, incompatible overload in its cache.

drop function if exists public.airs_get_dashboard_snapshot(text, text, integer);

create function public.airs_get_dashboard_snapshot(
  p_user_id uuid,
