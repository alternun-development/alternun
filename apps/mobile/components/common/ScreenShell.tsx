...
export function useScreenShellScroll(): ScreenShellContextType {
...
export default function ScreenShell({
  ...
  const setScrollRef = useCallback((ref: ScrollView | null): void => {
  ...
  return (
    <ScreenShellContext.Provider value={contextValue}>
      <ThemeProvider mode={isDark ? 'dark' : 'light'}>
        ...
        >
          {/* Main content — paddingTop follows the measured floating TopNav height */}
          <View
            style={[
              styles.body,
              {
                paddingTop: Math.max(topNavHeight > 0 ? topNavHeight + 16 : 0, isMobile ? 88 : 104),
              },
            ]}
          >
            {children}
