...
interface AppPreferencesContextValue {
  ...
  setShowWelcomeBonusModal: (show: boolean) => void;
...
export function AppPreferencesProvider({ children }: PropsWithChildren) {
  ...
  const [showWelcomeBonusModal, setShowWelcomeBonusModal] = useState<boolean>(
    typeof stored?.showWelcomeBonusModal === 'boolean' ? stored.showWelcomeBonusModal : true
  );
  ...
  const persistPreferences = (next: StoredPreferences) => {
    writeStoredPreferences({
      ...
      showWelcomeBonusModal:
        typeof next.showWelcomeBonusModal === 'boolean'
          ? next.showWelcomeBonusModal
          : showWelcomeBonusModal,
  ...
  const setShowWelcomeBonusModalWithPersist = (show: boolean) => {
    setShowWelcomeBonusModal(show);
    persistPreferences({ showWelcomeBonusModal: show });
  };
  ...
  const value = useMemo<AppPreferencesContextValue>(() => {
    ...
    return {
      ...
      setShowWelcomeBonusModal: setShowWelcomeBonusModalWithPersist,
      setMotionLevel: setMotionLevelWithPersist,
      toggleThemeMode,
      cycleLanguage,
      cycleMotionLevel,
    };
  }, [themeMode, language, showAirsIntro, showWelcomeBonusModal, motionLevel]);
