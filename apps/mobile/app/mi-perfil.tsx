...
function getProfileInfo(user: User | null,): { displayName: string; email?: string } {
  ...
  const firstName = typeof metadata.firstName === 'string' ? metadata.firstName : typeof metadata.first_name === 'string' ? metadata.first_name : '';
  const lastName = typeof metadata.lastName === 'string' ? metadata.lastName : typeof metadata.last_name === 'string' ? metadata.last_name : '';
  const fullNameFromParts = `${firstName} ${lastName}`.trim();
  const nameCandidates = [metadata.fullName, metadata.full_name, metadata.displayName, metadata.display_name, metadata.name, fullNameFromParts,];
  const validName = nameCandidates.find((entry,): entry is string => typeof entry === 'string' && entry.trim().length > 0,);
  const emailLocalPart = typeof user.email === 'string' && user.email.includes('@',) ? user.email.split('@',)[0] : undefined;
...
function getWalletAddress(user: User | null,): string {
  if (!user) return '';
  const metadata = getMetadata(user,);
  const walletObject = typeof metadata.wallet === 'object' && metadata.wallet !== null ? (metadata.wallet as UserMetadata) : undefined;
  const candidates = [metadata.walletAddress, metadata.wallet_address, metadata.address, walletObject?.address, walletObject?.walletAddress,];
  for (const candidate of candidates) {
    if (typeof candidate === 'string' && candidate.startsWith('0x',) && candidate.length >= 10) return candidate;
  }
  if (typeof user.providerUserId === 'string' && user.providerUserId.startsWith('0x',)) return user.providerUserId;
  return '';
}
...
function getWalletProvider(user: User | null,): string | null {
  if (!user) return null;
  const metadata = getMetadata(user,);
  const provider = typeof metadata.walletProvider === 'string' ? metadata.walletProvider : typeof metadata.wallet_provider === 'string' ? metadata.wallet_provider : null;
  if (provider && provider.trim().length > 0) return provider.toLowerCase();
  if (typeof user.provider === 'string' && user.provider.startsWith('wallet:',)) return user.provider.replace('wallet:', '',).toLowerCase();
  return null;
}
...
function getRoles(user: User | null,): string[] {
  ...
  if (Array.isArray(user.roles,)) for (const role of user.roles) if (typeof role === 'string' && role.trim().length > 0) roles.add(role.trim(),);
  const metadataRoles = metadata.roles;
  if (Array.isArray(metadataRoles,)) for (const role of metadataRoles) if (typeof role === 'string' && role.trim().length > 0) roles.add(role.trim(),);
  else if (typeof metadataRoles === 'string' && metadataRoles.trim().length > 0) roles.add(metadataRoles.trim(),);
...
function RankingTab({ isDark, c, }: { isDark: boolean; c: any },): React.JSX.Element {
  ...
  return (
    <ScrollView contentContainerStyle={[rankingStyles.content, { paddingBottom: 100, },]} showsVerticalScrollIndicator={false}>
      ...
      <SectionContainer title='Top Impactores'>
        <SearchFilterBar value={search} onChangeText={setSearch} placeholder='Buscar persona...' filters={RANKING_FILTERS} activeFilter={activeFilter} onChangeFilter={setActiveFilter} />
        <GlassCard style={rankingStyles.listCard}>
          {filteredUsers.map((user, idx,) => (
            <View key={user.rank} style={[rankingStyles.row, idx < filteredUsers.length - 1 && { borderBottomWidth: 1, borderBottomColor: c.border, },]}>
              <Text style={[rankingStyles.rankNum, { color: user.color ?? c.muted, },]}>{user.rank}</Text>
              <View style={[rankingStyles.avatarCircle, { backgroundColor: user.color ? `${user.color}22` : 'rgba(255,255,255,0.08)', borderColor: user.color ? `${user.color}44` : 'rgba(255,255,255,0.12)', },]}>
                <Text style={[rankingStyles.avatarText, { color: user.color ?? c.muted, },]}>{getInitials(user.name,)}</Text>
              </View>
...
function WalletTab({ isDark, c, }: { isDark: boolean; c: any },): React.JSX.Element {
  return (
    <ScrollView contentContainerStyle={[walletStyles.content, { paddingBottom: 100, },]} showsVerticalScrollIndicator={false}>
      <GlassCard style={walletStyles.connectCard}>
        ...
        <Text style={[walletStyles.connectSub, { color: c.muted, },]}>Vincula tu billetera para gestionar tus ATN tokens</Text>
        <TouchableOpacity style={[walletStyles.connectBtn, { backgroundColor: `${c.accent}18`, borderColor: `${c.accent}44`, },]} activeOpacity={0.7}>
...
function PerfilTab({ isDark, c, user, signingOut, onSignOut, router, }: { isDark: boolean; c: any; user: User | null; signingOut: boolean; onSignOut: () => void; router: any },): React.JSX.Element {
  ...
  return (
    <ScrollView contentContainerStyle={[profileStyles.content, { paddingBottom: 100, },]} showsVerticalScrollIndicator={false}>
      {/* Hero */}
      <View style={[profileStyles.heroCard, { backgroundColor: c.cardBg, borderColor: c.cardBorder, },]}>
        <View style={[profileStyles.avatarWrap, { backgroundColor: `${c.accent}14`, },]}>
          <Text style={[profileStyles.avatarText, { color: c.accent, },]}>{toInitials(profile.displayName,)}</Text>
        </View>
        <Text style={[profileStyles.heroName, { color: c.text, },]}>{profile.displayName}</Text>
        {profile.email && <Text style={[profileStyles.heroEmail, { color: c.muted, },]}>{profile.email}</Text>}
        <View style={[profileStyles.authPill, { backgroundColor: `${c.accent}14`, borderColor: `${c.accent}44`, },]}>
      ...
      {/* Account */}
      <View style={[profileStyles.infoCard, { backgroundColor: c.cardBg, borderColor: c.cardBorder, },]}>
      ...
      <View style={[profileStyles.infoCard, { backgroundColor: c.cardBg, borderColor: c.cardBorder, },]}>
        ...
        <ProfileInfoRow label='Status' value={walletConnected ? 'Connected' : 'Not Connected'} c={c} />
        {walletAddress && <ProfileInfoRow label='Address' value={truncateMiddle(walletAddress,)} c={c} />}
      ...
      <View style={profileStyles.actionRow}>
        <TouchableOpacity style={[profileStyles.secondaryBtn, { borderColor: c.cardBorder, backgroundColor: c.cardBg, },]} onPress={() => router.push('/settings',)} activeOpacity={0.7}>
          <SettingsIcon size={16} color={c.text} />
          <Text style={[profileStyles.btnText, { color: c.text, },]}>Settings</Text>
        </TouchableOpacity>
        <TouchableOpacity style={profileStyles.dangerBtn} onPress={onSignOut} disabled={signingOut} activeOpacity={0.7}>
          {signingOut ? (
...
function ProfileInfoRow({ label, value, c, }: { label: string; value: string; c: any },): React.JSX.Element {
  return (
...
export default function MiPerfilScreen(): React.JSX.Element {
  ...
  const c = isDark
    ? { bg: '#050f0c', cardBg: 'rgba(255,255,255,0.04)', cardBorder: 'rgba(255,255,255,0.08)', border: 'rgba(255,255,255,0.08)', text: '#e8fff6', muted: 'rgba(232,255,246,0.6)', accent: '#1EE6B5', }
    : { bg: '#f0fdf9', cardBg: 'rgba(255,255,255,0.85)', cardBorder: 'rgba(11,90,95,0.12)', border: 'rgba(11,90,95,0.12)', text: '#0b2d31', muted: 'rgba(11,45,49,0.6)', accent: '#0d9488', };
    ...
    return (
      <View style={[styles.center, { backgroundColor: c.bg, },]}>
        <GlassCard style={{ alignItems: 'center', }}>
          ...
          <Text style={[styles.signInPrompt, { color: c.text, },]}>Inicia sesión para ver tu perfil</Text>
          <TouchableOpacity style={[styles.signInBtn, { backgroundColor: `${c.accent}18`, borderColor: `${c.accent}44`, },]} onPress={() => router.push('/auth?next=/mi-perfil',)}>
    ...
    <ScreenShell activeSection='mi-perfil' backgroundColor={c.bg}>
      <View style={[styles.root, { backgroundColor: c.bg, },]}>
        ...
        <Animated.View style={[styles.tabContent, { opacity: fadeAnim, transform: [{ translateY: slideAnim, },], },]}>
          ...
          {activeTab === 'perfil' && <PerfilTab isDark={isDark} c={c} user={user} signingOut={signingOut} onSignOut={handleSignOut} router={router} />}
        </Animated.View>
      </View>
    </ScreenShell>
  );
...
const styles = StyleSheet.create({
  ...
  signInBtn: { marginTop: 16, paddingHorizontal: 20, paddingVertical: 10, borderRadius: 8, borderWidth: 1, },
...
const walletStyles = StyleSheet.create({
  ...
  walletIconWrap: { width: 88, height: 88, borderRadius: 44, alignItems: 'center', justifyContent: 'center', marginBottom: 16, },
  ...
  networkPill: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 14, paddingVertical: 8, borderRadius: 999, backgroundColor: 'rgba(255,255,255,0.04)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.08)', },
...
const profileStyles = StyleSheet.create({
  ...
  avatarWrap: { width: 56, height: 56, borderRadius: 28, alignItems: 'center', justifyContent: 'center', marginBottom: 12, },
  avatarText: { fontSize: 20, fontWeight: '700', },
  heroName: { fontSize: 18, fontWeight: '700', marginBottom: 4, },
  heroEmail: { fontSize: 13, marginBottom: 12, },
  authPill: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6, borderWidth: 1, flexDirection: 'row', alignItems: 'center', gap: 6, },
  ...
  secondaryBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 12, borderRadius: 8, borderWidth: 1, },
  dangerBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 12, borderRadius: 8, backgroundColor: 'rgba(255,59,48,0.1)', },
