...
## Local API startup: pooler tenant not found

`XX000` with `(ENOTFOUND) tenant/user ... not found` is a response from
Supabase's shared pooler, not necessarily a local DNS failure. Check whether
the development project is paused/inactive and resume it in Supabase before
retrying. If it is active, copy its complete current connection string from
...
local environment; keep credentials out of tracked files. Do not point local
development at production or disable migrations to hide an unavailable database.

Startup searches for `supabase/migrations` from the repository root or from
`apps/api` and fails if neither location exists. It must not silently report
success while applying no migrations because of the working directory.
