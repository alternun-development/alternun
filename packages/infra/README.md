...
The live testnet rollout is split across three deploy targets:

- `dev` ships the user-facing Expo bundle to `testnet.airs.alternun.co`
...
### Preferred Live Testnet Deploy Flow

For live testnet auth or UI changes, use the stage-aware deploy sequence that matched the working rollout:
...
- `INFRA_BACKEND_API_DATABASE_URL_SECRET_NAME` (optional override for the Secrets Manager secret used by migration/deploy helpers; defaults to `alternun/api/infra-backend-api-database-url-dev` for dev/testnet-like stages and `alternun/api/infra-backend-api-database-url-prod` for production-like stages)
...
- Dashboard/backend and identity CodeBuild jobs hydrate the Google and Discord OAuth client IDs and secrets from the stage-scoped integration config secret before infra synthesis, then pass them through as the `INFRA_IDENTITY_*` and `INFRA_BACKEND_API_*` env contracts. Expo-only bundle builds skip that resolver and keep the secret material out of the app path.
...
The SST deploy flow now also forces a post-deploy sync of the Expo web `dist/` tree into the live site bucket so the manifest and web bundle cannot stay stranded in an older S3 object set after a successful build.
...
- `alternun-admin` access is gated by the `alternun-admin-access` expression policy, which only allows users in `Alternun Dashboard Admins` or `authentik Admins` (`ALTERNUN_BOOTSTRAP_ADMIN_GROUP`) — a successful Google/Discord login does not grant admin access by itself; new admins must be added to one of those groups explicitly (see `docs/alternun-authentik-admin-sso-incident-2026-08.md`)
...
This script synchronizes the local branches and opens the production release PR automatically:

1. fetches `develop` and `master`
2. updates local `develop` and `master` from their remote counterparts
3. creates or reuses a pull request from `develop` to `master`

The PR still must be merged manually because branch protection blocks direct pushes to `master`.
