...
Testnet keeps `INFRA_IDENTITY_GOOGLE_LOGIN_FLOW_SLUG=alternun-google-login` so Google login uses the custom source-stage flow there, while localhost browser dev ignores custom flow slugs and falls back to direct source login.
