...
void test('buildspec sources the canonical SSM auth env helper and clears stale auth vars in CI', () => {
  ...
  assert.match(
    helperSource,
    /unset\s+\\[\s\S]*?EXPO_PUBLIC_API_URL[\s\S]*?EXPO_PUBLIC_SUPABASE_URL[\s\S]*?EXPO_PUBLIC_SUPABASE_KEY[\s\S]*?EXPO_PUBLIC_SUPABASE_ANON_KEY[\s\S]*?INFRA_BACKEND_API_SUPABASE_URL[\s\S]*?INFRA_BACKEND_API_SUPABASE_ANON_KEY[\s\S]*?AUTH_EXECUTION_PROVIDER[\s\S]*?EXPO_PUBLIC_AUTH_EXECUTION_PROVIDER/
  );
