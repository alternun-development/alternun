...
void test('expo bucket naming is stage-scoped and organized', () => {
  ...
  assert.match(expoConfigSource, /siteBucketName: string/);
  ...
  assert.match(infraConfigSource, /INFRA_EXPO_SITE_BUCKET_NAME: expoSiteBucketName/);
  ...
  assert.match(syncExpoSiteAssetsSource, /INFRA_EXPO_SITE_BUCKET_NAME/);
  assert.match(syncExpoSiteAssetsSource, /aws s3 sync/);
  assert.match(syncExpoSiteAssetsSource, /--delete/);
  assert.match(syncExpoSiteAssetsSource, /expo-web-site/);
});
