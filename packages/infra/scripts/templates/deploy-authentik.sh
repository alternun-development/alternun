...
MOBILE_OIDC_LAUNCH_URL="$(printf '%s' "${INTEGRATION_SECRET_JSON}" | jq -r '.mobileOidcLaunchUrl // empty')"
...
BOOTSTRAP_INTERNAL_USER_PROMOTION_MODE="$(printf '%s' "${INTEGRATION_SECRET_JSON}" | jq -r '.internalUserPromotionMode // "domain"')"
...
BOOTSTRAP_DOCS_CMS_OIDC_LAUNCH_URL="$(printf '%s' "${INTEGRATION_SECRET_JSON}" | jq -r '.docsCmsOidcLaunchUrl // empty')"
...
apply_authentik_source_stage_hotfix() {
  timeout --kill-after=3 60 "${compose_cmd[@]}" -f /opt/alternun/identity/docker-compose.yml exec -u root -T \
...
run_authentik_migrations() {
  ...
  while [ "${attempt}" -le "${max_attempts}" ]; do
    if timeout --kill-after=5 120 "${compose_cmd[@]}" -f /opt/alternun/identity/docker-compose.yml exec -T \
...
if ! BOOTSTRAP_RESULTS="$(timeout --kill-after=10 300 "${compose_cmd[@]}" -f /opt/alternun/identity/docker-compose.yml exec -T \
  ...
  -e ALTERNUN_BOOTSTRAP_INTERNAL_USER_PROMOTION_MODE="${BOOTSTRAP_INTERNAL_USER_PROMOTION_MODE}" \
  ...
  -e ALTERNUN_BOOTSTRAP_DOCS_CMS_OIDC_LAUNCH_URL="${BOOTSTRAP_DOCS_CMS_OIDC_LAUNCH_URL}" \
  ...
  -e ALTERNUN_BOOTSTRAP_MOBILE_OIDC_LAUNCH_URL="${MOBILE_OIDC_LAUNCH_URL}" \
