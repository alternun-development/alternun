  ...
  return {
    'dashboard-dev': {
      ...
      buildEnv: buildNonExpoPipelineEnv('dashboard-dev', ALL_PIPELINE_SET, {
        ...
        INFRA_BACKEND_API_SUPABASE_URL: env.EXPO_PUBLIC_SUPABASE_URL ?? '',
    ...
    'dashboard-prod': {
      ...
      buildEnv: buildNonExpoPipelineEnv('dashboard-prod', ALL_PIPELINE_SET, {
        // dashboard-prod enables the backend API without deploying identity
        // infrastructure in the same stack, so identityInfrastructure is undefined
        // here and infra.config.ts's authentikIntegrationConfigSecretArn can only
        // come from this variable. Without it, deployBackendApiInfrastructure falls
        // back to the retired, non-existent alternun-infra/identity/integration-config/
        // identity-prod path and the stack update fails with "couldn't find resource"
        // even though the admin site itself deploys fine. (INFRA_IDENTITY_SECRET_
        // INTEGRATION_CONFIG_NAME only feeds buildIdentitySettings for a stack that
