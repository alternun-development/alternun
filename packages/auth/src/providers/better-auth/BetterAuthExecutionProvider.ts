...
function isVerifiedEmailSignup(result: unknown): boolean {
  if (!isRecord(result)) {
    return false;
  }
...
function isEmailVerificationPendingResult(result: AuthExecutionResult): boolean {
  return (
    result.needsEmailVerification === true ||
    result.confirmationEmailSent === true ||
    result.externalIdentity?.emailVerified === false
  );
}
...
export class BetterAuthExecutionProvider implements AuthExecutionProvider {
  ...
  async signIn(options: AuthExecutionSignInOptions): Promise<AuthExecutionResult> {
    ...
    if (client?.signIn) {
      ...
      if ((result as Record<string, unknown>)?.error) {
        ...
        throw new AlternunProviderError(
          typeof errorPayload.message === 'string' ? errorPayload.message : 'Sign in failed'
        );
      }
      ...
      const needsEmailVerification =
        provider === 'email'
          ? typeof (result as Record<string, unknown>)?.needsEmailVerification === 'boolean'
            ...
            : normalizedSession?.externalIdentity?.emailVerified === false
          : undefined;
    ...
    if (browserClient?.signIn) {
      if (provider === 'email' && browserClient.signIn.email && options.password) {
        ...
        if ((result as Record<string, unknown>)?.error) {
          const errorPayload = (result as Record<string, unknown>).error as Record<string, unknown>;
          throw new AlternunProviderError(
            typeof errorPayload.message === 'string' ? errorPayload.message : 'Email sign in failed'
          );
        }
    ...
    const needsEmailVerification =
      provider === 'email'
        ? typeof (response as Record<string, unknown>)?.needsEmailVerification === 'boolean'
          ...
          : session?.externalIdentity?.emailVerified === false
        : undefined;
    const confirmationEmailSent =
      provider === 'email'
  ...
  async signUp(input: AuthExecutionSignUpInput): Promise<AuthExecutionResult> {
    ...
    if (client?.signUp) {
      ...
      if ((result as Record<string, unknown>)?.error) {
        const errorPayload = (result as Record<string, unknown>).error as Record<string, unknown>;
        throw new AlternunProviderError(
          typeof errorPayload.message === 'string' ? errorPayload.message : 'Sign up failed'
        );
      }
    ...
    if (browserClient?.signUp?.email) {
      ...
      if ((result as Record<string, unknown>)?.error) {
        const errorPayload = (result as Record<string, unknown>).error as Record<string, unknown>;
        throw new AlternunProviderError(
          typeof errorPayload.message === 'string' ? errorPayload.message : 'Email sign up failed'
        );
      }
  ...
  async signInWithEmail(email: string, password: string): Promise<User> {
    ...
    if (browserClient?.signIn?.email) {
      ...
      const authResult: AuthExecutionResult = {
        session,
        externalIdentity: session?.externalIdentity ?? null,
        needsEmailVerification:
          typeof (result as Record<string, unknown>)?.needsEmailVerification === 'boolean'
      ...
      if (isEmailVerificationPendingResult(authResult)) {
        throw new AlternunProviderError(
          'Email not confirmed. Please check your inbox before signing in.'
        );
      }
