...
- The canonical browser-facing Better Auth URL is the API-origin `/auth` route. In local development, that route proxies to the private service on `http://localhost:9083`. Deployed stacks can also derive that base URL from `AUTH_EXCHANGE_URL` / `EXPO_PUBLIC_AUTH_EXCHANGE_URL` when the explicit Better Auth URL is omitted.
