...
async function main() {
  ...
  if (!response.ok) {
    throw new Error(`Supabase config update failed (${response.status}): ${responseText}`);
  }
  ...
  const report = {
    ...
    payload: {
      ...resolved.payload,
      smtp_pass_configured: Boolean(resolved.payload.smtp_pass),
    },
    response: parsedResponse,
  };

  const outputPath = path.join(getOutputDirectory(), 'supabase-sync-report.local.json');
