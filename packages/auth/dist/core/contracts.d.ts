import type { AlternunSession, AuthExecutionResult, AuthExecutionSignInOptions, AuthExecutionSignUpInput, AuthLinkProviderInput, AuthRuntimeConfig, AuthUnlinkProviderInput, ClaimValidationResult, EmailMessageInput, EmailProviderHealthcheckResult, EmailProviderName, ExecutionSession, IdentityExchangeInput, IdentityIssuerProviderName, IssuerDiscoveryConfig, IssuerSession, LinkedAuthAccount, Principal, PrincipalRecord, ProvisioningEventRecord, UserProjectionRecord, AuthExecutionProviderName, WalletConnectionBridge, ExternalIdentity } from './types';
export interface AuthLoggerEvent {
    channel: 'execution-provider' | 'issuer-exchange' | 'repository-upsert' | 'email-provider' | 'compatibility' | 'runtime';
    action: string;
...
export interface IdentityIssuerProvider {
    ...
    validateClaims(claims: Record<string, unknown>, options?: {
        issuer?: string;
