...
function normalizeBrowserLocaleCandidate(value: string | null | undefined): AlternunLocale | null {
  if (!value) {
    return null;
  }
...
}: ResolveBrowserLocaleOptions = {}): AlternunLocale {
  const candidates = [documentLang, ...(navigatorLanguages ?? []), navigatorLanguage];
...
export function getCatalog(locale: string | null | undefined): TranslationCatalog {
